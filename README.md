...
| `revision/wp2_monotonic_sensitivity.py` | `revision/wp2_monotonic_sensitivity.json` | Secs 3.4, 4.6 — label counts and monotonic-completion sensitivity (Table II). Note: the JSON's raw Stage-marker tallies predate the reclassification of the borderline condition (60 km/h, ISO 1600, 3.5 m) as expert-revised; the manuscript's 37 / 7 / 6 composition counts that condition under expert-verified, so the JSON shows 6 expert-verified plus one untagged condition |
...
| `revision/A7_underexposure_band_brightness.py` | `revision/A7_underexposure_band_brightness.json` | Sec 4.7 — per-frame concrete-band mean grey levels of the two under-exposed conditions (6.5 m, ISO 100, both speeds): condition means 28.4 and 30.1 DN, reported as 28-30 DN. Requires the Tier-3 cam2 frame archive (on request); the JSON is the fixed record |
